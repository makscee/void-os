...
// and clear any chats.current_run_id pointer that referenced them, in a
// single transaction so the chats table never lags behind runs.
//
...
export function bootRecovery(
  ...
  const tx = db.transaction(() => {
    ...
    db.run(
      "UPDATE chats SET current_run_id = NULL WHERE current_run_id IN (SELECT id FROM runs WHERE status = 'interrupted')",
    );
  });
  tx();
