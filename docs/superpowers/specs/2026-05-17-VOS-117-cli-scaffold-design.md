...
3. Resolve vault: `--vault` > `VOID_OS_VAULT_ROOT` env > daemon's own default (`~/Library/Application Support/void-os/vault`).
...
7. Poll `GET /health` every 200 ms for up to 10 s.
...
3. `kill(pid, SIGTERM)`. Poll up to 5 s for exit. Still alive → `kill(pid, SIGKILL)`, wait up to 2 s.
4. Remove `daemon.pid` and `daemon.port`. Print `stopped`. Exit 0.
...
- Human: write `content` raw to stdout (no trailing newline added). Binary-flagged responses (4xx `E_BINARY` from daemon) → exit 1 with `binary file, use --json`.
...
- `vault read/write/list`: against running daemon in tmp vault; round-trip a file.
